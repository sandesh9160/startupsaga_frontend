...
/* ---------------------------------------------
   Convert theme object → CSS styles
--------------------------------------------- */
...
function themeToStyleObject(theme: ThemeSettings): React.CSSProperties {
  ...
  for (const [key, value] of Object.entries(theme)) {
    ...
    style[cssVar] = value; // no `any`
  }

  if (theme.background_color || theme.bg_color) {
    style.background =
      theme.background_color || theme.bg_color;
  }
...
/* ---------------------------------------------
   Convert theme → data attributes
--------------------------------------------- */

function themeToDataAttrs(
  theme: ThemeSettings
): Record<string, string> {
  const attrs: Record<string, string> = {};
  if (theme.dropdown_style) {
    attrs["data-dropdown-style"] = theme.dropdown_style;
  }
  return attrs;
}

/* ---------------------------------------------
   Route → Theme Resolver
--------------------------------------------- */
...
function pathToThemeParams(pathname: string): {
  pageKey?: string;
  pageSlug?: string;
} {
  ...
  if (!RESERVED_SLUGS.includes(first) && segments.length === 1) {
    return { pageSlug: first };
  }
...
export function ThemeProvider({
  ...
  const {
    pageKey: pathPageKey,
    pageSlug: pathPageSlug,
  } = pathToThemeParams(pathname || "");
  ...
  /* ------------------------------
     Fetch theme when route changes
  ------------------------------ */
  ...
  useEffect(() => {
    ...
    getThemeSettings({ pageKey, pageSlug })
      .then(setTheme)
      .catch(() => setTheme({}));
  }, [pageKey, pageSlug, initialTheme]); // ✅ dependency fixed
